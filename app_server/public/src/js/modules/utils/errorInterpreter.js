...
const validationErrors = {
  any: {
    unknown: 'is not a possible option.',
    invalid: 'contains an invalid value.',
    empty: 'is not allowed to be empty.',
    required: 'is required.',
    allowOnly: 'must be one of {{valids}}.',
    default: 'threw an error when running the default method.'
  },
  alternatives: {
    base: 'is not matching any of the allowed alternatives.'
  },
  array: {
    base: 'must contain multiple values.',
...
const formatErrors = function (errors) {
  let propercase = (str) => str.substr(0, 1).toUpperCase() + str.substr(1).toLowerCase();
  return errors.map((err) => propercase(err.msg.startsWith('!!') ?
